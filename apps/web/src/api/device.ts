...
export async function fetchDevices(params: DeviceQuery) {
  const { data } = await request.get<ApiResponse<Paginated<DeviceItem>>>("/devices", {
...
export async function deleteDevice(id: string) {
  const { data } = await request.delete<ApiResponse<unknown>>(`/devices/${id}`);
  return data.data;
}
...
export async function fetchDeviceHistory(id: string) {
  const { data } = await request.get<ApiResponse<unknown>>(`/devices/${id}/history`);
  return data.data;
}
