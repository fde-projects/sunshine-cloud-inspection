...
export class ExcelParserService {
  ...
  async parseGspCases(buffer: Buffer) {
    ...
    if (!headerRow) throw new Error('未找到GSP案例表头（需含「服务案例号」）');
  ...
  async parseSettlePrices(buffer: Buffer) {
    ...
    if (!sheet) throw new Error('价格文件中没有工作表');
    const scenes = [
      { name: '平地', hours: 11 },
      { name: '水上', hours: 12 },
      { name: '山地', hours: 13 },
  ...
  async parsePerfPrices(buffer: Buffer) {
    ...
    if (!sheet) throw new Error('绩效价文件中没有工作表');
    ...
    if (!cCode || !cPrice) {
      throw new Error('未识别到「条目编码」「单价」列，请使用绩效价导入清单模板');
    }
