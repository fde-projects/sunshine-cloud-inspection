...
export const HARD_RULE_DEFAULTS: HardRuleSeed[] = [
  ...
  },
  {
    ...
    promptText: [
      ...
      '',
      '如何认「当前选中」：只看页签/按钮的选中态（高亮、下划线、填充色），不要把未选中页签上的文字标题当成已打开。',
      '列表有带日期的历史条目，通常是「历史故障」；仅当选中「实时故障」且内容为实时列表/暂无数据时才算实时。',
      '',
      '必须不合格：',
      '只交一张；两张都是同一页签（例如两张都是历史故障）；只看见页签标题但没点开对应内容。一张图上同时出现「实时故障」和「历史故障」标题，只算当前选中的那一页，另一页没拍。设备首页、发电量等不算。',
      '禁止为了凑齐两种页签而把两张同页签图分别标成实时与历史。',
    ].join('\n'),
  },
  {
    code: 'sungrow',
  ...
  },
  {
    ...
    promptText: [
      '【安装固定检查·硬性否决】',
      '有合格样时：以合格样种类为准（如整机/抱箍/线缆），现场须对齐全部分种类且达到样张同级；顺序不限。',
      '无合格样时，至少 2 张有效且不重复的照片，并同时具备两类证据（可在不同照片上）：',
      'A) 整机/安装关系概览：能看清设备与电杆/墙面/支架的相对位置；',
      'B) 固定点特写：能看清抱箍、U型螺栓或锁紧螺母本体。',
      'A、B 由不同照片分担即可，禁止要求每一张都同时具备 A+B。',
      '整机概览、抱箍特写、线缆特写属于不同拍摄类型，不算构图重复。',
      '仅见机箱外壳/线管、看不清锁紧点的照片，不能当作 B 类；但若已另有照片对上「抱箍」合格样，不得因此否决整机组。',
      '拿不准是否拍齐合格样种类时，按不合格处理。',
    ].join('\n'),
  },
];
